...
import { Clock, FolderOpen, Plus, Sparkles, Users, Zap } from 'lucide-react';
import Avatar from 'boring-avatars';
import { useAuthStore } from '@/store/auth.store';
import { BRAND_AVATAR_PALETTE } from '@/lib/constants';

const STATS = [
  { label: 'Total Projects', value: '12', icon: FolderOpen, bg: 'bg-pink-50', color: 'text-brand-primary' },
  { label: 'Team Members', value: '4', icon: Users, bg: 'bg-purple-50', color: 'text-purple-500' },
  { label: 'Active Files', value: '38', icon: Zap, bg: 'bg-sky-50', color: 'text-sky-500' },
  { label: 'This Month', value: '6', icon: Sparkles, bg: 'bg-amber-50', color: 'text-amber-500' },
];
...
const RECENT_PROJECTS = [
  ...
  },
  {
    ...
    updatedAt: 'Yesterday',
    collaborators: ['Ali Osaid', 'Omar N', 'Rami K'],
  ...
  },
  {
    ...
    updatedAt: 'Last week',
    collaborators: ['Ali Osaid', 'Sara M', 'Khalid J'],
      ...
      <div className="mb-8">
        ...
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4">
          {RECENT_PROJECTS.map((project) => (
            ...
            >
              <div className={`relative h-28 bg-linear-to-br ${project.gradient}`}>
                <div className="absolute inset-0 flex items-center justify-center">
                  <div className="grid grid-cols-3 gap-1.5 opacity-15">
                    {Array.from({ length: 6 }).map((_, i) => (
                      <div key={i} className="h-6 w-10 rounded bg-white" />
                    ))}
                  </div>
                </div>
              </div>
